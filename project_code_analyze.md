...
<ul> 1) CascadeClassifier로 얼굴인식 학습결과 파일을 불러온다</ul>
<ul>    load_model로 h5 또는 hdf5(표정인식 학습결과 파일)로 저장된 모델 구조를 불러온다</ul>
<ul>    화남,  역겨운, 두려운,  행복, 슬픈, 놀란, 무표정 값이 들어간 EMOTIONS 배열생성 </ul></br></br>
        
<ul> 2) camera = cv2.VideoCapture(0)으로 웹 캠을 이용하여 비디오 재생</ul>
<ul>      while문으로 계속하여 재생되는 비디오를 한 프레임 씩 읽어들인다</ul>
<ul>      ret, frame = camera.read() 정상적인 값을 받으면, ret가 True이고, read 함수로 비디오를 한 프레임씩 읽어들임</ul> </br></br>
        
<ul> 3) 배경과 현재 입력 프레임(frame)과의 차영상을 이용하여 객체를 검출해야 하기 때문에</ul>
<ul>    cvtColor(frame, cv2.COLOR_BGR2GRAY)로 이미지(배경)를 회색조로 변경한다</ul>
<ul>    face_detection.detectMultiScale로 얼굴을 먼저 검출한다 (아직 표정을 예측하는게 아님)</ul> 
        <ol><ul>* gray = 대상 이미지</ul>
        <ul>* scaleFactor=1.1 = 이미지 크기</ul>
        <ul>* minNeighbors=5 = 얼굴 검출 후보들의 개수 </ul>
        <ul>* minSize=(30,30) = 가능한 최소 객체(얼굴) 사이즈 </ul>
        </ol> </br></br>
       
<ul> 4) 감정 예측 데이터 값을 나타낼 검정색 캔버스 생성해준다 </ul>
<ul>    zeros()는 numpy의 배열을 생성해주는 함수</ul></br></br>

<ul> 5) 만약에 3)에서 얼굴이 검출된 값이 들어있는 faces가 0이상일 때 즉, 이미지를 검출했을 때 </ul>
<ul>    검출한 이미지를 face에 정렬하여 넣어준다 </ul>
